...
nlohmann::json InitializeJson(std::string input, std::default_random_engine generator, const std::time_t &time, int frequency, int totalFreqs)
{
	nlohmann::json freq;
	...
	freq["frequency"] = 900 + (totalFreqs*5) + frequency;
	freq["time"] = time;
...
std::string Message(std::string input, std::default_random_engine generator, int number, const std::chrono::time_point<std::chrono::system_clock> &time, int totalFreqs)
{
	...
	for (int i = 0; i < 5; i++)
	{
		freqs.push_back(InitializeJson(input, generator, tTime, i, totalFreqs));
	}
